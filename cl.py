...
class MNLCoefficients:
    ...
    def add_shared(
        ...
        if isinstance(items_indexes, int):
            ...
            self.add_coefficients(
                coefficient_name, feature_name, items_indexes, items_names
            )
    ...
    def _add_tf_weight(self, weight_name, weight_index):
        ...
        if (
            self.coefficients[weight_name]["feature_name"]
            in self.feature_to_weight.keys()
        ):
            self.feature_to_weight[
                self.coefficients[weight_name]["feature_name"]
            ].append(
    ...
    def get_weight_item_indexes(self, feature_name):
        ...
        return [
            self.coefficients[weight_name]["items_indexes"]
...
class ConditionalLogit(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(
            add_exit_choice=add_exit_choice, optimizer=optimizer, lr=lr, **kwargs
        )
    ...
    def _add_coefficient(
        self, feature_name, coefficient_name, items_indexes, items_names, shared
    ):
        ...
        coefficient_name = (
            coefficient_name if coefficient_name else "beta_%s" % feature_name
        )
    ...
    def instantiate(self, choice_dataset):
        ...
        if not self.instantiated:
            if not isinstance(self.coefficients, MNLCoefficients):
                self._build_coefficients_from_dict(
                    n_items=choice_dataset.base_num_items
                )
            ...
            if choice_dataset.items_features_by_choice_names is not None:
                for feat_tuple in choice_dataset.items_features_by_choice_names:
                    dataset_stacked_features_names.append(feat_tuple)
            dataset_stacked_features_names = np.concatenate(
                dataset_stacked_features_names
            ).ravel()
    ...
    def _store_dataset_features_names(self, choice_dataset):
        ...
        """
        self._shared_features_by_choice_names = (
            choice_dataset.shared_features_by_choice_names
        )
        self._items_features_by_choice_names = (
            choice_dataset.items_features_by_choice_names
        )
    ...
    def compute_batch_utility(
            ...
            for i, feat_tuple in enumerate(self._shared_features_by_choice_names):
                for j, feat in enumerate(feat_tuple):
                    if feat in self.coefficients.features_with_weights:
                        ...
                        for item_index, weight_index in zip(
                            item_index_list, weight_index_list
                        ):
                            partial_items_utility_by_choice = tf.zeros(
                                (n_choices, n_items)
                            )
                            ...
                            items_utilities_by_choice.append(
                                tf.cast(
                                    tf.stack(partial_items_utility_by_choice, axis=1),
                                    tf.float32,
                                )
                            )
                    elif verbose > 0:
        ...
        if "intercept" in self.coefficients.features_with_weights:
            item_index_list, weight_index_list = (
                self.coefficients.get_weight_item_indexes("intercept")
            )
        ...
        with tf.GradientTape() as tape_1:
            with tf.GradientTape(persistent=True) as tape_2:
                ...
                loss = tf.keras.losses.CategoricalCrossentropy(reduction="sum")(
                    ...
                    y_true=tf.one_hot(
                        choice_dataset.choices, depth=probabilities.shape[1]
                    ),
                )
    ...
    def clone(self):
        ...
        if hasattr(self, "_shared_features_by_choice_names"):
            clone._shared_features_by_choice_names = (
                self._shared_features_by_choice_names
            )
